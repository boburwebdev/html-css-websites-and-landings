$(function(){
    
    $('.about__slider').slick({
        prevArrow: `<button class="slider__btn slider__btn_left"><svg width="10" height="18" viewBox="0 0 10 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.21839 1L1 9L9.21839 17"/></svg></button>`,
        nextArrow: `<button class="slider__btn slider__btn_right"><svg width="10" height="18" viewBox="0 0 10 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0.78161 17L9 9L0.78161 1"/></svg></button>`,
    });

})